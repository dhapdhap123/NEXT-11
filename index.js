...
// gps 정보 가져올 수 있을 때 가져오고, weather_request 실행
...
function weather_request(x_lat, y_lon) {
  const { year, month, date, hour, minute } = getTime();
  ...
  url_make_request(url_long, API_KEY, request_day, request_time, x_lat, y_lon);
  url_make_request(url_short, API_KEY, request_day, request_time, x_lat, y_lon);
}

// whole url_long 만들고 request, response 값 array로 가져옴
function url_make_request(
  url,
  API_KEY,
  request_day,
  request_time,
  ...
  const whole_url = `${url}?serviceKey=${API_KEY}&pageNo=1&numOfRows=1000&dataType=JSON&base_date=${request_day}&base_time=${request_time}&nx=${x_lat}&ny=${y_lon}`;
  if (url === url_long) {
    make_long_array(whole_url);
  } else {
    make_short_array(whole_url);
  }
}
...
function make_short_array(whole_url) {
  fetch(whole_url);
  // 이어 만들기
}

function make_long_array(whole_url) {
  fetch(whole_url)
    .then((response) => {
