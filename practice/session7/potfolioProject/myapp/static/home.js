...
초단기 예보, 초단기 실황 API
*/
const GEO_API_KEY = "a700dddcebfc4132fe2d5d905ed9b0cb";
...
function onGeoOk(position) {
  ...
  const url = `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=${GEO_API_KEY}&units=metric`;
  fetch(url)
    .then((response) => {
...
function fetch_short_wheather(whole_url, request_time) {
  fetch(whole_url)
    ...
    .then((item) => {
      ...
      container.innerHTML += `현재 온도 : ${item_obj.T1H}도, `;
    });
}
...
function fetch_long_wheather(whole_url, request_time) {
  fetch(whole_url)
    ...
    .then((item) => {
      ...
      const container = document.getElementById("weather");
      const sky_value = item_obj[0].SKY;
      switch (sky_value) {
        case "1":
          container.innerHTML += `현재 날씨 : 맑음`;
          break;
        case "3":
          container.innerHTML += `현재 날씨 : 구름 조금 낌`;
          break;
        case "4":
          container.innerHTML += `현재 날씨 : 구름 많이 낌`;
          break;
        default:
          console.log("오류!");
          break;
...
const url = "../static/images/home/" + images[num];
...
imgElement.style.height = "calc(100% - 5px)";
